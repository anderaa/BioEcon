...
# we'll use this to fill in dataframe
exitMarker <- rep(exitMarker, daysAlive)
exit1Marker <- rep(exit1Marker, daysAlive)
exit2Marker <- rep(exit2Marker, daysAlive)
exit3Marker <- rep(exit3Marker, daysAlive)
exit4Marker <- rep(exit4Marker, daysAlive)
exit5Marker <- rep(exit5Marker, daysAlive)
exit7Marker <- rep(exit7Marker, daysAlive)
exit8Marker <- rep(exit8Marker, daysAlive)

vaccMarker <- rep(dateOfVacc, daysAlive)
standMarker <- rep(standOfRes, daysAlive)

# combine above vectors into a dataframe
mortalityData <- data.frame(dogID, male, day, age, exit, exit1, exit2, exit3, exit4, exit5, exit7, exit8)

# increment day and age, set mark day of exit for those dogs that exit
for(i in 2:nrow(mortalityData)) {
  if(dogID[i] == dogID[i-1]) {
    mortalityData$day[i] <- mortalityData$day[i-1] + 1
  } else {
    mortalityData$exit[i-1] <- exitMarker[i-1]
    mortalityData$exit1[i-1] <- exit1Marker[i-1]
    mortalityData$exit2[i-1] <- exit2Marker[i-1]
    mortalityData$exit3[i-1] <- exit3Marker[i-1]
    mortalityData$exit4[i-1] <- exit4Marker[i-1]
    mortalityData$exit5[i-1] <- exit5Marker[i-1]
    mortalityData$exit7[i-1] <- exit7Marker[i-1]
    mortalityData$exit8[i-1] <- exit8Marker[i-1]
  }
}
...
# Stands
stand <- standMarker
mortalityData <- cbind(stand, mortalityData)
...
# calculating average litters per female per year
litterData <- mortalityData[mortalityData$age>=300, ]
fertileDays <- nrow(litterData)
litterData$birthMarker <- as.numeric(litterData$birthMarker)
totalLitters <- sum(litterData$birthMarker)
fertileYears <- fertileDays/365

littersPerFemPerYear <- totalLitters/fertileYears
littersPerFemPerYear
########################################################################################################################
